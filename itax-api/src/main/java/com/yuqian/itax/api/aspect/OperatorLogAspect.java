package com.yuqian.itax.api.aspect;
...
public class OperatorLogAspect {
    ...
    @Pointcut("execution(public * com.yuqian.itax.api.controller..*(..))")
	...
	public Object handle(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		log.info("【请求 oemCode】：{}，【请求 sourceType】：{}，【请求 token】：{}", request.getHeader("oemCode"), request.getHeader("sourceType"), request.getHeader("token"));
		...
		try {
			...
			if (resultMessage.contains("base64")) {
		...
		//添加操作日志
		operationLogService.addOperationLog(1, request.getRemoteAddr(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), resultMessage, request.getHeader("oemCode"), request.getHeader("token"));
		return result;
	}
